...
| `migrations/0045_drop_reviews.sql` | Removes seller reviews from the product. Re-points existing review reports at the review's author (`target_type = 'user'`) so moderation history survives, rebuilds `reports_queue` without the review columns, drops `reports.target_review_id` and restores `reports_target_type_check` / `reports_target_present` to their 0011 shape, rewrites `enforce_report_rate_limit()` so its duplicate guard stops comparing the dropped column, then drops `public.reviews` (taking its policies, index, grants and 0032's filter trigger) and `enforce_review_content()`. No `CASCADE`, so an unknown dependency fails loudly. **Irreversible: deletes every review row.** Apply **after** 0044. |
| `migrations/0046_remove_24h_report_promise.sql` | Withdraws the 24-hour report response promise. Redefines `enforce_report_rate_limit()` with only its duplicate-report message changed (the app shows it verbatim, and it was the one copy of the promise living in Postgres), and rewrites `notify_new_report()`'s comment, which described the alert as backing that promise. The in-app copy changes alongside it, and `report-alert` now also emails the reporter a confirmation. Apply **after** 0045. |
