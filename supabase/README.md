...
| `tests/report_reviews_test.sql` | Report-a-review tests (0044): a review report is accepted carrying both the review and its author, an unknown `target_type` is still rejected, a report naming no target at all is still rejected (the widened constraint did not loosen), the rebuilt queue joins the review body/rating/seller, the view is still revoked from `anon`/`authenticated` and still readable by `service_role`, and deleting a review cascades to its reports. |
