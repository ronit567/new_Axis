...
| R16 | 2.3.3 | you | open | Screenshots at 6.9" and 6.5" showing the app in use, never the login or splash screen. **Plus a 13" iPad set** — required now that `supportsTablet` is true; App Store Connect will not accept the build for iPad without it. Take them in both orientations' worth of real content, not a stretched phone layout. Use a Release build, not `npx expo start`, so no development overlay appears. On a Mac stuck on Xcode 26.3 (macOS below 26.6) SDK 57 will not compile locally, so build in the cloud with `eas build --profile preview-simulator --platform ios` and install with `eas build:run --platform ios --profile preview-simulator --latest`. iPhone 17 Pro Max gives the 6.9" size (1320 × 2868); iPad Pro 13-inch gives 13" (2064 × 2752). |
