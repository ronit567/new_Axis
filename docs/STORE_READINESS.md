...
| R16 | 2.3.3 | you | done | Screenshots uploaded 2026-09-22: three at iPhone 6.5" and a 13" iPad set. 6.5" alone satisfies the iPhone requirement -- App Store Connect accepts either 6.9" or 6.5", so the "and" this row used to demand was wrong. The iPad set was required because `supportsTablet` is true. |
...
| R25 | 2.3.6 | agent | done | `TermsOfServiceScreen` set eligibility by email domain only and stated no minimum age, while the App Store listing is rated 13+. Eligibility now reads "You must be at least 13 years old" and says accounts found to belong to someone under 13 are removed, which matches the privacy policy's Children section and the 13+ that R15's questionnaire produced. Last-updated bumped to 2026-09-22. **The copy at `dataaxis.org/terms` is a separate document and still carries the old wording** -- the two are linked from inside the app and diverging them is its own 1.2 problem, so update the site to match. |
...
| R30 | LIVE | you | done | `npx supabase migration list` on 2026-09-22 shows `0001`-`0052` applied in production, with `0048`-`0052` -- including the `0052` whose absence broke the chat screen on 2026-09-20 -- all present. Conversation delete and per-listing threads work against the live database. |
...
4. **R24** — decide on image moderation: ship as disclosed, or add screening.
5. **R25** — done in the app; update `dataaxis.org/terms` to match the new minimum-age clause.
6. **R27** — confirm the published contact address reaches the report queue.
7. **R28** — Sentry org, project and auth token into the EAS environments.
8. **R26** — no action unless App Review asks; keep the institutional-name answer ready.
