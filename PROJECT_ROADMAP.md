...
**Tasks:**
- `reports_queue` SQL view (migration `0012_reports_queue.sql`) over AX-703's `reports` table — joins in reporter/target names + emails + the reported listing's title, ordered open-first, revoked from `anon`/`authenticated` so it's reachable only from the Studio SQL editor, which connects as `postgres` (no in-app moderation UI in v1; note `service_role` holds no SELECT on these tables and cannot read the view). Satisfies "reports land in a queryable queue (SQL view/dashboard OK for v1)".
- `CommunityGuidelinesScreen` Reporting section now states the 24h response commitment + `axis.app@outlook.com` (the same contact already on `TermsOfServiceScreen`) — the "contact" leg of Guideline 1.2.
